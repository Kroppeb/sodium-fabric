...
public interface ChunkRenderBackend<T extends ChunkGraphicsState> {
    ...
    default void beforeRender(MatrixStack matrixStack, double x, double y, double z, SodiumWorldRenderer sodiumWorldRenderer, ChunkRenderManager<T> tChunkRenderManager, Matrix4f projectionMatrix) {
    }

    default void setVisibilityData(ChunkSectionPos sectionPos, ChunkOcclusionData occlusionData){
    }

    default void setVisibilityData(ChunkSectionPos sectionPos, int occlusionData){
    }
}
